...
We illustrate the problem above again:
...
Particularly, we do not force the robots to be at a specific location at a given time.
This means that, as part of our plannign problem, we also plan when a robot can be somewhere in order to avoid colliding with other robots

Thus, multi-goal path planning enables something like this{% include sidenote.html text='visiting the same state, and going to a final collision free configuration B'%}:

[..]

So far, I always did multi robot-multi goal _path-planning_, i.e. neglecting all kinds of dynamics constraints, and only minimizing a proxy to time.
...
If we formulate the problem above as optimization problem, we have to deal with the question of when do we actually want to reach the goal?
To figure out how we can deal with this, I briefly recap some trajectory optimization{% include sidenote.html text='[This tutorial](https://www.matthewpeterkelly.com/tutorials/trajectoryOptimization/index.html) is in my opinion by far the best resource to get an intro to the topic, anything I write here is a worse summary.'%}.
...
In pracice there are some additional constraints, i.e. collision constraints, possibly restricting the magnitude of the input $$u$$, and bounding the state $$x$$.
...
The most generic one is to assume a fixed timestep in order to discretize the state and input trajectory, and approximating all the relevant continuous functions.
...
This is called direct collocation, and is the classic version of it, and results in the optimization problem
...
We can just as normally discretize the state, and add a constraint on the state that corresponds to the time at which we want to fulfill our constraint.
...
We could instead discretize all robot trajectories separately, and give each robot their own $$\Delta t$$ as well, which would allow for moving the intermediate times around as we wish.
...
This means that when solving this, we need to take some special care.

# Literature

We are here writing up a few related things to the problem of introducing time in our optimization problem.
I am of course not the first persion thinking about introducing time in the optimizatino problems, altough most previous work had different reasons for doing it.
...
A common theme is bilevel optimization where we first do path planning, and then time parametereize the plan
